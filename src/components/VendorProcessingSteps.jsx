/* eslint-disable react/prop-types */
/* eslint-disable no-unused-vars */
import React, { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { CheckCircle2, Circle, Loader2 } from 'lucide-react';

const steps = [
  'Uploading vendor quotes',
  'Parsing vendor pricing',
  'Comparing lane costs',
  'Evaluating vendor reliability',
  'Running AI matching engine'
];

export function VendorProcessingSteps({ onComplete }) {
  const [currentStep, setCurrentStep] = useState(0);

  useEffect(() => {
    if (currentStep < steps.length) {
      const timer = setTimeout(() => {
        setCurrentStep((prev) => prev + 1);
      }, 1500 + Math.random() * 800);
      return () => clearTimeout(timer);
    } else {
      setTimeout(onComplete, 800);
    }
  }, [currentStep, onComplete]);

  return (
    <div className="w-full max-w-md mx-auto space-y-4 py-8 relative">
      <div className="absolute left-[19px] top-6 bottom-6 w-0.5 bg-white/10" />
      <AnimatePresence>
        {steps.map((step, index) => {
          const isCompleted = currentStep > index;
          const isCurrent = currentStep === index;
          const isPending = currentStep < index;

          if (isPending) return null;

          return (
            <motion.div
              key={step}
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              className="relative flex items-center space-x-4 pl-2"
            >
              <div className="relative z-10 bg-slate-900 p-1 rounded-full border border-slate-800 shadow-inner">
                {isCompleted ? (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="text-primary font-bold"
                  >
                    <CheckCircle2 className="w-6 h-6" />
                  </motion.div>
                ) : isCurrent ? (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="text-secondary"
                  >
                    <Loader2 className="w-6 h-6 animate-spin" />
                  </motion.div>
                ) : (
                  <Circle className="w-6 h-6 text-slate-600" />
                )}
              </div>
              <motion.span
                animate={{
                  color: isCompleted ? '#64748b' : '#ffffff',
                  fontWeight: isCurrent ? 600 : 400
                }}
                className="text-[17px] tracking-wide transition-colors"
              >
                {step}
                {isCurrent && <motion.span
                  initial={{ opacity: 0 }}
                  animate={{ opacity: [0, 1, 0] }}
                  transition={{ repeat: Infinity, duration: 1.5 }}
                >...</motion.span>}
              </motion.span>
            </motion.div>
          );
        })}
      </AnimatePresence>
    </div>
  );
}
